...
void lcd_draw(u8* pixels, int x, int y, int width, int height) {
  ...
  lcd_write_data(pixels, width * height / 2);
  // LSB_FIRST not supported
  /*spi_set_format(spi1, 16, 0, 0, SPI_MSB_FIRST);
  spi_write16_blocking(spi1, (u16*)pixels, width * height / 4);
...
void lcd_fill(u8 color, int x, int y, int width, int height) {
  ...
  color &= 0x7;
  //u8 color2 = ((color << 3) | color);
  //for (int i = 0; i < width * height / 2; i++) spi_write_blocking(spi1, &color2, 1);

  spi_set_format(spi1, 16, 0, 0, SPI_MSB_FIRST);
  u16 buf[256];
  int remaining = width * height / 4 + 1;
  int chunk_size = remaining < 256 ? remaining : 256;
  for (int i = 0; i < chunk_size; i++) buf[i] = (color << 11) | (color << 8) | (color << 3) | color;
...
u8 glyph_buf[GLYPH_WIDTH * GLYPH_HEIGHT / 2] __attribute__((aligned(4))); 
...
void lcd_draw_char(int x, int y, u8 fg, u8 bg, char c) {
  ...
  for (int j = 0; j < GLYPH_HEIGHT; j++) {
    for (int i = 0; i < GLYPH_WIDTH; i++) {
      int mask = 1 << i;
      //if (i & 1) glyph_buf[(i + j * GLYPH_WIDTH) / 2] |= ((font.glyphs[offset] & mask) ? fg : bg) << 3;
      if (i % 2 == 0) glyph_buf[(i + j * GLYPH_WIDTH) / 2] = ((font.glyphs[offset] & mask) ? fg : bg) << 3;
      else glyph_buf[(i + j * GLYPH_WIDTH) / 2] |= ((font.glyphs[offset] & mask) ? fg : bg);
    }
    offset++;
  }
...
void lcd_init() {
  ...
  lcd_write_reg(0x3A, 0x22);                // Pixel Interface Format 0x01 for 3-bit colour 
